...
引用については、[専用の脚注ページを作成し](https://chibicode.github.io/the-prize-research/)、そちらへのリンクを引用部分に貼ることで本文との[主従関係を示す](http://www.bunka.go.jp/chosakuken/naruhodo/ref.asp)形を取った。一段落すべてが引用であるのが明らかな場合、リンクは段落の最初か最後だけに貼っている。引用箇所は最小限にとどめ、原著よりもネット上の記事のほうが詳しければそちらの引用を優先している。詳しくは文化庁「著作権なるほど質問箱」の「[引用](http://www.bunka.go.jp/chosakuken/naruhodo/ref.asp)」の定義を[参考に](https://twitter.com/chibicode/status/834506990726418432)。また、「引用ではなくパクリでは」と思われる方もいると思う。ぼくはそうではないと思うが、まずは原著を読んでから判断していただきたい。
